...
exports.createTable = async (tablename, sql) => {
  // 建表
  let status = false
...
exports.queryUsernameAndEmail = async JSONdata => {
  // 这里用户名和邮箱要两个表都查，回调地狱写法太深了，用了 async/await Promise 重构
...
exports.insertReg = async (JSONdata, ip) => {
  // 插入数据表
  let status = [false, false]
